...
//RakuOrm.storeClass(Post)
//RakuOrm.storeClass(User)
RakuOrm.init(Post)
RakuOrm.init(User)
  ...
  describe('save()', () => {
   ...
   it('ids should not overlap even in conditions of high contention', () => {
     ...
     return raku.cset('User:last_id', 0)
       ...
       .then(() => raku.cget('User:last_id'))
       .then(last_id => expect(last_id).to.eql(N))
   })
...
describe('has_many relationship', () => {
  ...
  describe('user.load("post_ids")', () => {
    ...
    it("deleted posts should be deleted from the user's post_ids", () => {
      let user = new User()
      user.first_name = 'Loader'
      user.post_ids = [542, 1001]
      ...
      return user.save()
        .then(() => raku.get('Post#42:User:post_ids'))
        .then(post_ids => expect(post_ids).to.include(542))
        //.then(() => post.delete())
        //.then(() => User.load(user.id, 'post_ids', 'first_name'))
        //.then(user => {
        //  expect(user.post_ids).to.eql([1001])
        //})
    })
...
//
//    it('user.save() should change the array on disk', () => {
//      assert.fail('To be implemented.')
//    })
//
//    it('user.save() should save the back link to that user in the corresponding post's instance data.', () => {
//      if user 42 has post 13, make sure after user.save(), Post#13:User.post_ids contains 42
//      assert.fail('To be implemented.')
//    })
//
//  }) // describe post_ids
